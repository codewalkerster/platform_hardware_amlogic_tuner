...
void* VectorImpl::editArrayImpl()
{
    if (mStorage) {
        ...
        if (editable == 0) {
            ...
            // for which we're not the only owner, which is undefined behavior.
...
dp_state VectorImpl::sort(VectorImpl::compar_t cmp)
{
    return sort(sortProxy, (void*)cmp);
}

dp_state VectorImpl::sort(VectorImpl::compar_r_t cmp, void* state)
{
