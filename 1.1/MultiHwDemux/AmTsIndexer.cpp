...
static void find_mpeg(uint8_t *data, int len, TS_Indexer_t *indexer, TSParser *stream)
{
  ...
  for (i = 0; i < haystack_len - sizeof(needle) + 1;) {
    if (left < 5) {
      //ALOGD("MPEG2 picture header across TS Packet\n");
      ...
      if (p != NULL)
      {
        ...
        if (nal_unit_type == NAL_TYPE_IDR) {
          if (slice_type == 2 || slice_type == 7) {
            ...
            ALOGE("%s line%d invalid slice_type: %d, offset: %llx\n", __func__, __LINE__, slice_type, event.offset);
            nalu += nalu_len;
            continue;
          }
          ...
          if (slice_type == 0 || slice_type == 5) {
              ...
              ALOGE("%s line%d invalid slice_type: %d\n", __func__, __LINE__, slice_type);
              nalu += nalu_len;
              continue;
          }
        } else {
...
static void find_h265(uint8_t *data, int len, TS_Indexer_t *indexer, TSParser *stream)
{
    ...
    while (nalu != NULL) {
      ...
      if (nalu[0] == 0x00 && nalu[1] == 0x00 && nalu[2] == 0x01) {
        ...
        ALOGD("0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x, nalu_type: %d, offset: %#llx\n",
...
static void pes_packet(TS_Indexer_t *ts_indexer, uint8_t *data, int len, TSParser *stream)
{
  uint8_t *p = data;
  ...
  if (stream->PES.len > 0) {
    //ALOGD("%s have cache data %d bytes\n", __func__, stream->PES.len);
  ...
  if (stream->PES.state == TS_INDEXER_STATE_TS_START) {
    ...
    if (left < 6) {
      ...
      ALOGE("not enough ts payload len: %#x\n", left);
      return;
    }
    ...
    if ((p[0] != 0) || (p[1] != 0) || (p[2] != 1)) {
      ...
      ALOGE("%s, not the expected start code!\n", __func__);
      return;
    }
  ...
  if (stream->PES.state == TS_INDEXER_STATE_PES_HEADER) {
    if (left < 8) {
      ...
      ALOGE("not enough optional pes header len: %#x\n", left);
      return;
    }
...
/*Parse the TS packet.*/
static void ts_packet(TS_Indexer_t *ts_indexer, uint8_t *data)
{
    //ALOGD("[%s/%d]", __FUNCTION__, __LINE__);
  ...
  if (afc & 2) {
    ...
    if (len < 0) {
      ALOGD("illegal adaption field length!");
      return;
    }
  }
...
AmTsIndexer::AmTsIndexer() {
    ALOGD("new AmTsIndexer");
    mpts_indexer = new TS_Indexer_t();
    memset(mpts_indexer, 0, sizeof(TS_Indexer_t));
    ts_indexer_init(mpts_indexer);
}

AmTsIndexer::~AmTsIndexer() {
    ALOGD("release AmTsIndexer");
}

int AmTsIndexer::ts_indexer_init(TS_Indexer_t *ts_indexer) {
    TSParser init_parser;

    if (ts_indexer == NULL) {
        ALOGD("[%s/%d] ts_indexer not init", __FUNCTION__, __LINE__);
        return -1;
    }
    ...
    init_parser.format = TS_INDEXER_VIDEO_FORMAT_NONE;
 ...
 */
void AmTsIndexer::ts_indexer_destroy() {
    ALOGD("[%s/%d]", __FUNCTION__, __LINE__);

    mpts_indexer->callback = NULL;
    mpts_indexer->user_data = NULL;
 ...
 * \retval -1 On error.
 */
int AmTsIndexer::ts_indexer_set_video_format(TS_Indexer_StreamFormat_t format) {
    if (mpts_indexer == NULL)
        return -1;

    mpts_indexer->video_parser.format = format;
 ...
 * \retval -1 On error.
 */
int AmTsIndexer::ts_indexer_set_video_pid(int pid) {
    if (mpts_indexer == NULL)
        return -1;

    TSParser *parser = &mpts_indexer->video_parser;
    parser->pid = pid;
    parser->offset = 0;
    parser->PES.pts = -1;
    parser->PES.offset = 0;
 ...
 * \retval -1 On error.
 */
int AmTsIndexer::ts_indexer_set_audio_pid (int pid) {
    if (mpts_indexer == NULL)
        return -1;

    TSParser *parser = &mpts_indexer->audio_parser;
    parser->pid = pid;
    parser->offset = 0;
    parser->PES.pts = -1;
    parser->PES.offset = 0;
 ...
 * \retval -1 On error.
 */
int AmTsIndexer::ts_indexer_set_event_callback (TS_Indexer_EventCallback_t callback, void *user_data) {
    mpts_indexer->callback = callback;
    mpts_indexer->user_data = user_data;
    return 0;
}
/**
 ...
 */
int AmTsIndexer::ts_indexer_parse (uint8_t *data, int len) {
    uint8_t *p = data;
    int left = len;

    if (mpts_indexer == NULL || data == NULL  || len <= 0) {
          ALOGD(" mpts_indexer is null");
          return -1;
     }
    //ALOGD("[%s/%d]", __FUNCTION__, __LINE__);
    ...
    while (left > 0) {
      ...
      if (*p == 0x47) {
        if (left < TS_PKT_SIZE) {
          ALOGE("%s data length may not be 188-byte aligned\n", __func__);
          return left;
        }

        // parse one ts packet
        ts_packet(mpts_indexer, p);
        p += TS_PKT_SIZE;
        left -= TS_PKT_SIZE;
        mpts_indexer->offset += TS_PKT_SIZE;
      } else {
        p++;
        left--;
        mpts_indexer->offset++;
