        ...
        if (stream_id == PRIVATE_STREAM_1 || stream_id == PRIVATE_STREAM_2) {
            ...
            if (needSkipData) {
                ...
                do {
                    ...
                    result = AmDmxDevice->AM_DMX_Read(filterId, pesData.data() + 6 + 3 + pesHeaderLen + readLen, &dataLen);
...
Return<void> Demux::openDvr(DvrType type, uint32_t bufferSize, const sp<IDvrCallback>& cb,
    ...
    switch (type) {
        case DvrType::PLAYBACK:
            ...
            mDvrPlayback = new Dvr(type, bufferSize, cb, this, mTunerService);
        ...
        case DvrType::RECORD:
            ...
            mDvrRecord = new Dvr(type, bufferSize, cb, this, mTunerService);
...
Return<Result> Demux::connectCiCam(uint32_t ciCamId) {
    ...
    if (AmDmxDevice != NULL) {
        AmDmxDevice->AM_DMX_SetSource(0, INPUT_DEMOD, FRONTEND_TS1);
    }
    return Result::SUCCESS;
}
...
Return<Result> Demux::disconnectCiCam() {
    ...
    if (AmDmxDevice != NULL) {
        AmDmxDevice->AM_DMX_SetSource(0, INPUT_DEMOD, FRONTEND_TS2);
    }
    return Result::SUCCESS;
}
...
void Demux::startBroadcastTsFilter(vector<uint8_t> data) {
        ...
        if (isValidTsPacket(data)) {
            while (dvr_playback_write(mPlaybackhandle, data.data(), data.size()) == -1) {
                usleep(100 * 1000);
                if (mDvrPlayback && mDvrPlayback->stopInjectTs()) {
                    ALOGD("[demux] stop Inject TS, break!");
                    break;
                }
                ALOGD("[Demux][dmxId = %d] wait for 100ms to write dvr device", mDemuxId);
            }
...
void Demux::sendFrontendInputToRecord(vector<uint8_t> data, uint16_t pid, uint64_t offset, uint64_t pts, int iFrameIndex,int pusiIndex) {
...
int Demux::recordTsPacketForTemiData(uint64_t filterId) {
    mFilters[filterId]->stop();
    mTemiFid = filterId;
    //mAmDvrDevice[mDemuxId]->AM_DVR_SetCallback(postDvrData, this);
    ...
    AmDmxDevice->AM_DMX_AllocateFilter(&mTemiRecordFid);

    if (AmDmxDevice->AM_DMX_SetBufferSize(mTemiRecordFid, 10 * 1024 * 1024) != 0) {
        ALOGE("record AM_DMX_SetBufferSize");
        return -1;
    }
    if (AmDmxDevice->AM_DMX_SetPesFilter(mTemiRecordFid, &pparam) != 0) {
        ALOGE("record AM_DMX_SetPesFilter");
        return -1;
    }
    if (AmDmxDevice->AM_DMX_StartFilter(mTemiRecordFid) != 0) {
...
void Demux::closeTemiRecordFilter() {
    ...
    if (AmDmxDevice != NULL) {
        AmDmxDevice->AM_DMX_StopFilter(mTemiRecordFid);
        AmDmxDevice->AM_DMX_FreeFilter(mTemiRecordFid);
    }
...
sp<Descrambler> Demux::getDescrambler() {
