...
::ndk::ScopedAStatus Frontend::scan(const FrontendSettings& in_settings, FrontendScanType in_type) {
    ...
    */
    mFeDev->scan(in_settings, in_type);
    ...
    for (int i = 0; i < in_statusTypes.size(); i++) {
        ...
        switch (type) {
            ...
            case FrontendStatusType::SYMBOL_RATE: {
                status.set<FrontendStatus::symbolRate>(mFeDev->getSymbolRate());
                break;
            }
            ...
            case FrontendStatusType::LNB_VOLTAGE: {
                status.set<FrontendStatus::lnbVoltage>(mFeDev->getLnbVoltage());
                break;
            }
            ...
            case FrontendStatusType::LNA: {
                status.set<FrontendStatus::isLnaOn>(mFeDev->getLna());
                break;
            }
            ...
            case FrontendStatusType::RF_LOCK: {
                status.set<FrontendStatus::isRfLocked>(mIsLocked);
                break;
            }
