...
#ifndef STAGEFRIGHT_FOUNDATION_TSP_DATA_H_
#define STAGEFRIGHT_FOUNDATION_TSP_DATA_H_

#include <memory> // for std::shared_ptr, weak_ptr and unique_ptr
#include <type_traits> // for std::aligned_union

#include <utils/RefBase.h>// for android::sp and wp
...
/**
 * TSPData is a flexible union type that supports non-POD members. It supports arbitrary types as long
 * as they are either moveable or copyable.
 *
 * Internally, TSPData is using AUnion - a structure providing the union support. TSPUnion should not
 * be used by generic code as it is very unsafe - it opens type aliasing errors where an object of
 * one type can be easily accessed as an object of another type. TSPData prevents this.
 *
 * TSPData allows a custom type flagger to be used for future extensions (e.g. allowing automatic
 * type conversion). A strict and a relaxed flagger are provided as internal types.
 *
 * Use as follows:
 *
 * TSPData<int, float>::Basic data; // strict type support
 * int i = 1;
 * float f = 7.0f;
 *
 * data.set(5);
 ...
 * EXPECT_EQ(f, 6.0f);
 *
 * TSPData<int, sp<RefBase>>::RelaxedBasic objdata; // relaxed type support
...
/**
 * Helper class to call constructor and destructor for a specific type in TSPUnion.
...
template<>
HIDE inline void _TSPUnion_impl::emplace<void>(size_t totalSize, void *addr) {
    memset(addr, 0, totalSize);
}

/** Destructor specialization for void type */
template<>
HIDE inline void _TSPUnion_impl::del<void>(void *) {
}
...
public:
    ...
    template<
            ...
            typename=typename std::enable_if<is_one_of<T, void, Ts...>::value>::type>
    inline void del() {
        _TSPUnion_impl::del(reinterpret_cast<T*>(&mValue));
    }
 ...
 */
template<typename T>
struct HIDE _TSPData_copier<T, false> {
 ...
 */
template<typename T>
struct HIDE _TSPData_mover<T, false> {
...
/**
 * Helper template that deletes an object of a specific type (member) in an TSPUnion.
 *
 * \param Flagger type flagger class (see TSPData)
...
struct HIDE _TSPData_deleter<Flagger, U> {
 ...
 *
 * \param Flagger type flagger class (see TSPData)
 * \param U TSPUnion object in which the member should be copy assigned
 * \param Ts types to consider for the member
 */
template<typename Flagger, typename U, typename ...Ts>
struct HIDE _TSPData_copy_assigner;

/**
 * Template specialization when there are still types to consider (T and rest)
 */
template<typename Flagger, typename U, typename T, typename ...Ts>
struct HIDE _TSPData_copy_assigner<Flagger, U, T, Ts...> {
    static bool assign(typename Flagger::type flags, U &dst, const U &src) {
        ...
        return _TSPData_copy_assigner<Flagger, U, Ts...>::assign(flags, dst, src);
    }
};
 ...
 */
template<typename Flagger, typename U>
struct HIDE _TSPData_copy_assigner<Flagger, U> {
 ...
 */
template<typename Flagger, typename U, typename ...Ts>
struct HIDE _TSPData_move_assigner;

/**
 * Template specialization when there are still types to consider (T and rest)
 */
template<typename Flagger, typename U, typename T, typename ...Ts>
struct HIDE _TSPData_move_assigner<Flagger, U, T, Ts...> {
    ...
    assign(typename Flagger::type flags, U &dst, U &src) {
        ...
        return _TSPData_move_assigner<Flagger, U, Ts...>::assign(flags, dst, src);
    }
 ...
 */
template<typename Flagger, typename U>
struct HIDE _TSPData_move_assigner<Flagger, U> {
 ...
 *
 * TSPData<types...>::Basic   - strict type support using uint32_t flag.
 *
 * TSPData<types...>::Strict<Flag> - strict type support using custom flag.
 * TSPData<types...>::Relaxed<Flag, MaxSize, Align>
 ...
 *                            safety.
 *
 * TSPData<types...>::RelaxedBasic - strict type support using uint32_t flag.
 *
 * TSPData<types...>::Custom<flagger> - custom type support (flaggers determine the supported types
...
public:
    ...
    /**
     * Implementation of TSPData.
     */
    template<typename Flagger>
    struct Custom : protected Flagged<TSPUnion<Ts...>, typename Flagger::type, Flagger::mask> {
        using data_t = TSPUnion<Ts...>;
        using base_t = Flagged<TSPUnion<Ts...>, typename Flagger::type, Flagger::mask>;
        ...
        Custom& operator=(const Custom &o) {
            if (&o != this) {
                ...
                if (o.used()) {
                    if (_TSPData_copy_assigner<Flagger, data_t, Ts...>::assign(
        ...
        Custom& operator=(Custom &&o) {
            if (&o != this) {
                ...
                if (o.used()) {
                    if (_TSPData_move_assigner<Flagger, data_t, Ts...>::assign(
                            o.flags(), this->get(), o.get())) {
                        this->setFlags(o.flags());
                        o.clear();
                    } else {
        ...
        bool clear() {
            if (this->used()) {
                if (_TSPData_deleter<Flagger, data_t, Ts...>::del(this->flags(), this->get())) {
        ...
        bool find(T *data) const {
            ...
            if (this->flags() == Flagger::flagFor((T*)0) ||
                ...
                _TSPData_copier<T>::assign(data, this->get().template get<B>());
                return true;
            }
            return false;
        }
        ...
        bool remove(T *data) {
            ...
            if (this->flags() == Flagger::flagFor((T*)0) ||
                Flagger::canDeleteAs(this->flags(), Flagger::flagFor((T*)0))) {
                _TSPData_mover<T>::assign(data, std::move(this->get().template get<B>()));
                return true;
            }
            return false;
        }
        ...
        bool set(const T &data) {
            ...
            if (this->flags() == Flagger::flagFor((T*)0) && std::is_same<T, B>::value) {
                _TSPData_copier<B>::assign(&this->get().template get<B>(), data);
                return true;
            } else if (this->used()) {
        ...
        bool set(T &&data) {
            ...
            // if already contains an object of this type, simply assign
            if (this->flags() == Flagger::flagFor((T*)0) && std::is_same<T, B>::value) {
                _TSPData_mover<B>::assign(&this->get().template get<B>(), std::forward<T&&>(data));
                return true;
            } else if (this->used()) {
                // destroy previous object
                if (!this->clear()) {
    ...
    /**
     * Basic TSPData using the default type flagger and requested flag type.
     *
    ...
    /**
     * Basic TSPData using the default type flagger and uint32_t flag.
     */
    using Basic = Strict<uint32_t>;

    /**
     * TSPData using the relaxed type flagger for max size and requested flag type.
     *
     * \param Flag desired flag type to use. Must be an unsigned and std::integral type.
     */
    template<typename Flag, size_t MaxSize = 1024, size_t Align = 4>
    ...
    /**
     * Basic TSPData using the relaxed type flagger and uint32_t flag.
     */
