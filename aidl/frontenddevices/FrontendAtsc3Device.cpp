...
int FrontendAtsc3Device::getFrontendSettings(FrontendSettings *settings, void * fe_params) {
    ...
    if (settings->getTag() != FrontendSettings::Tag::atsc3) {
        return -1;
    }

    p_fe_params->frequency = settings->get<FrontendSettings::Tag::atsc3>().frequency;

    if (settings->get<FrontendSettings::Tag::atsc3>().bandwidth == FrontendAtsc3Bandwidth::UNDEFINED) {
        settings->get<FrontendSettings::Tag::atsc3>().bandwidth = FrontendAtsc3Bandwidth::AUTO;
    }
    p_fe_params->u.ofdm.bandwidth = getFeAtsc3BandwidthType(settings->get<FrontendSettings::Tag::atsc3>().bandwidth);

    if (settings->get<FrontendSettings::Tag::atsc3>().demodOutputFormat == FrontendAtsc3DemodOutputFormat::UNDEFINED) {
        settings->get<FrontendSettings::Tag::atsc3>().demodOutputFormat = FrontendAtsc3DemodOutputFormat::ATSC3_LINKLAYER_PACKET;
    }
    /*p_fe_params->u.ofdm.demodOutputFormat =
        (fe_demodOutputFormat_t)(getFeAtsc3DemodOutputFormatType(settings->get<FrontendSettings::Tag::atsc3>().demodOutputFormat));
    */

    int len = settings->get<FrontendSettings::Tag::atsc3>().plpSettings.size();
    for (int i = 0; i < len; i++) {
        if (settings->get<FrontendSettings::Tag::atsc3>().plpSettings[i].modulation == FrontendAtsc3Modulation::UNDEFINED) {
            settings->get<FrontendSettings::Tag::atsc3>().plpSettings[i].modulation = FrontendAtsc3Modulation::AUTO;
        }

        if (settings->get<FrontendSettings::Tag::atsc3>().plpSettings[i].interleaveMode == FrontendAtsc3TimeInterleaveMode::UNDEFINED) {
            settings->get<FrontendSettings::Tag::atsc3>().plpSettings[i].interleaveMode = FrontendAtsc3TimeInterleaveMode::AUTO;
        }

        if (settings->get<FrontendSettings::Tag::atsc3>().plpSettings[i].codeRate == FrontendAtsc3CodeRate::UNDEFINED) {
            settings->get<FrontendSettings::Tag::atsc3>().plpSettings[i].codeRate = FrontendAtsc3CodeRate::AUTO;
        }

        if (settings->get<FrontendSettings::Tag::atsc3>().plpSettings[i].fec == FrontendAtsc3Fec::UNDEFINED) {
            settings->get<FrontendSettings::Tag::atsc3>().plpSettings[i].fec = FrontendAtsc3Fec::AUTO;
        }
    }
    return 0;
}
